...
class Internship_PointController extends Controller
{
    public function show($id)
    {
        ...
        if ($role->name === "Admin" || $role->name === "Trợ lý đào tạo") {
            $data = DB::select('SELECT ip.product,ip.teacher_comment,ip.company_comment,ip.id,s.mssv,u.name,t.name as topic_name,v.teacher_name,v.company_name,ip.teacher_point,ip.company_comment,ip.company_point,ip.total_point 
            FROM users u 
            ...
            $data = DB::select('SELECT ip.product,itime.course, itime.semester, itime.year, ip.teacher_comment,ip.company_comment,ip.id,s.mssv,u.name, itype.name as name_type,DATE_FORMAT(itime.start_time, "%d/%m/%Y") as start_time,DATE_FORMAT(itime.end_time, "%d/%m/%Y") as end_time,t.name as topic_name,v.teacher_name,v.company_name,ip.teacher_point,ip.company_comment,ip.company_point,ip.total_point 
            FROM users u 
            JOIN students s ON s.id_user = u.id 
            JOIN student_reg sr ON sr.id_student = s.id
    ...
    public function create(Request $request)
    {
        ...
        return response()->json($Internship_Point, 200);
    }
