        ...
        <div id="sampleTable_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
            <b-container fluid>
                <div class="card-title text-center">
                    ...
                    <!-- end -->
                    <p class="card-category">Xem thông tin, thêm, sửa, xóa học phần</p>
                ...
                <b-row class="mx-1 my-2 float-y">
                    ...
                    <b-button-group size="sm">
                        ...
                        <b-button variant="success" @click="$refs.exportExcel.$el.click()">
                            ...
                            <vue-excel-xlsx class="d-none" ref="exportExcel" :data="items" :columns="fields" :filename="'DS học phần thực tập CNTT'" :sheetname="'sheetname'"></vue-excel-xlsx>
                        </b-button>
                    </b-button-group>
                </b-row>
                ...
                <b-modal id="modal-insert" ref="modal" centered size="md" hide-footer hide-header>
                    <b-form @submit.prevent="insert">
                        <div class="row">
                            <h6 class="text-center mt-2 col-10">Thêm học phần mới:</h6>
                            <i @click="hide_modal" class="fa fa-times col-2 text-right" aria-hidden="true"></i>
                            <div class="form-group mb-0 col-12">
                                <label class="control-label col-form-label-sm">Tên học phần:</label>
                                <input class="form-control form-control-sm" type="text" name="name" v-model.trim="internship.name" />
                            </div>
                ...
                <b-modal id="modal-update" ref="modal" centered size="md" hide-header hide-footer>
                    <b-form @submit.prevent="update">
                        <div class="row">
                            <h6 class="text-center mt-2 col-10">Cập nhật thông tin học phần:</h6>
                            <i @click="hide_modal" class="fa fa-times col-2 text-right" aria-hidden="true"></i>
...
export default {
    ...
    methods: {
        ...
        getData() {
            $("#overlay").fadeIn(300);
            this.$http.get('api/internship_type').then(response => {
                $("#overlay").fadeOut(300);
        ...
        changeStatus(id) {
            $("#overlay").fadeIn(300);
            this.$http.get('api/internship_type/status/' + id).then(response => {
                $("#overlay").fadeOut(300);
            }, response => {
                this.getData();
            })
        ...
        insert() {
            $("#overlay").fadeIn(300);
            this.$http.post('api/internship_type', this.internship).then(response => {
                this.getData();
                this.resetData();
        ...
        getUpdate(id) {
            $("#overlay").fadeIn(300);
            this.$http.get('api/internship_type/' + id).then(response => {
                $("#overlay").fadeOut(300);
        ...
        update() {
            $("#overlay").fadeIn(300);
            this.$http.put('api/internship_type/' + this.internship.id, this.internship).then(response => {
                this.getData();
                this.resetData();
                this.$refs['modal'].hide();
                this.$noty.success("Thành công :)");
        ...
        del(id) {
            this.$swal({
                ...
                if (result.value) {
                    $("#overlay").fadeIn(300);
                    this.$http.delete("api/internship_type/" + id).then(
                        response => {
                            this.getData();
                            this.$noty.success("Thành công :)");
                        },
